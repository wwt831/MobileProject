...
public class XmlParse extends DefaultHandler {
	...
	public void startElement(String uri, String localName, String qName,
		...
		if (information) {
			if (tagName.equals(Constant.CITY_NAME_PATH)) {
				baseWeather.setCityName(attributes
						.getValue(Constant.ATTRIBUTE_NAME));
			}
			if (tagName.equals(Constant.FIRST_DATE_PATH)) {
				String[] date = attributes.getValue(Constant.ATTRIBUTE_NAME)
						.split("-");
				int day = Integer.valueOf(date[2]);
				day--;
				date[2] = String.valueOf(day);
				this.dateTime = new StringBuilder().append(date[0]).append("-")
						.append(date[1]).append("-").append(date[2]).toString();
			}
		}
	...
	public void endElement(String uri, String localName, String qName)
		...
		if (tagName.equals(Constant.FORECAST_CONDITION)) {
			inForcast = false;
			weather.setCurrently_date(Utils.getNextDate(dateTime));
